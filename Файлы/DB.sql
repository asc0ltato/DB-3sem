...
--код производителя (поле MFR из таблицы ORDERS)
     (MFR_ID CHAR(3) NOT NULL,
--айди продукта (поле PRODUCT из таблицы ORDERS) 
...
--код офиса (поле REP_OFFICE из таблицы SALESREPS) 
     (OFFICE INT NOT NULL,
--город (нет)
        CITY VARCHAR(15) NOT NULL,
--регион (нет)
      REGION VARCHAR(10) NOT NULL,
--сотрудник, который руководит данным офисом
--(поле EMPL_NUM из таблицы SALESREPS, поле MGR из таблицы OFFICES, поле CUST_REP из таблицы CUSTOMERS, поле REP из таблицы ORDERS)
         MGR INT,
--сколько должны наторговать (поле QUOTA из таблицы SALESREPS)
      TARGET DECIMAL(9,2),
--сколько всего наторговали (поле SALES из таблицы SALESREPS)
...
--коды сотрудников 
--(поле MGR из таблицы OFFICES, поле CUST_REP из таблицы CUSTOMERS, поле REP из таблицы ORDERS)
...
--возраст(нет)
         AGE INTEGER,
--код офиса(поле OFFICE из таблицы OFFICES)
  REP_OFFICE INTEGER,
--должность(нет)
       TITLE VARCHAR(10),
--дата, когда его взяли(нет)
   HIRE_DATE DATE NOT NULL,
--начальник сотрудника
--(поле MGR из таблицы OFFICES, поле CUST_REP из таблицы CUSTOMERS, поле REP из таблицы ORDERS)
     MANAGER INT,
--сколько должны наторговать(поле TARGET из таблицы OFFICES)
       QUOTA DECIMAL(9,2),
--сколько наторговал определенный сотрудник(поле SALES из таблицы OFFICES)
       SALES DECIMAL(9,2) NOT NULL,
...
--номер покупателя (поле CUST из таблицы ORDERS)
   (CUST_NUM INTEGER    NOT NULL,
--название компании(нет)
    COMPANY VARCHAR(20) NOT NULL,
--какой сотрудник закреплен за данной организацией
--(поле MGR из таблицы OFFICES, поле EMPL_NUM из таблицы SALESREPS, поле REP из таблицы ORDERS)
    CUST_REP INTEGER,
--хз(нет)
...
--хранятся заказы, оформляют кто-то из продавцов
CREATE TABLE ORDERS
--номер заказа(нет)
  (ORDER_NUM INTEGER NOT NULL,
             CHECK (ORDER_NUM > 100000),
--дата оформления заказа(нет)
  ORDER_DATE DATE NOT NULL,
--номер покупателя (поле CUST_NUM из таблицы CUSTOMERS)
        CUST INTEGER NOT NULL,
--номер продавца 
--(поле MGR из таблицы OFFICES,поле CUST_REP из таблицы CUSTOMERS, поле EMPL_NUM из таблицы SALESREPS)
         REP INTEGER,
--код производителя (поле MFR_ID из таблицы PRODUCTS)
         MFR CHAR(3) NOT NULL,
--айди продукта (поле PRODUCT_ID из таблицы PRODUCTS)
     PRODUCT CHAR(5) NOT NULL,
--число заказанного(нет)
         QTY INTEGER NOT NULL,
--итоговая сумма заказанного(нет)
